  ...
  * @param swap The Swap struct containing address of the decentralized exchange (DEX) and calldata containing data for interacting with the DEX during the execution.
   ...
   */
    function executeBuyTwap(uint256 strategyId, Swap calldata swap) external {
        ...
        if (!LibTime.getTimeDifference(block.timestamp, strategy.buyTwapExecutedAt, timeToExecute)) {
            revert ExpectedTimeNotElapsed();
        }

        uint256 value = executionBuyValue(strategyId);

        transferBuy(strategyId, value, swap, price, investRoundId, stableRoundId, strategy.buyAt);
   ...
   */
    ...
    function executeBTD(
        ...
        checkRoundDataMistmatch(strategyId, fromInvestRoundId, fromStableRoundId, toInvestRoundId, toStableRoundId);
        (uint256 price, uint80 investRoundId, uint80 stableRoundId) = LibPrice.getPrice(
            strategy.parameters._investToken,
            strategy.parameters._stableToken
        );
        ...
        transferBuy(strategyId, value, swap, price, investRoundId, stableRoundId, price);
        if (!strategy.parameters._sell && !strategy.parameters._floor && strategy.parameters._stableAmount == 0) {
            strategy.status = Status.COMPLETED;
        }
    }
    ...
    function executionBuyValue(uint256 strategyId) public view returns (uint256) {
        ...
        if (strategy.parameters._buyDCAUnit == DCA_UNIT.FIXED) {
            value = (strategy.parameters._stableAmount > strategy.parameters._buyDCAValue)
    ...
    function transferBuy(
        ...
        if (strategy.parameters._floor && strategy.floorAt > 0 && strategy.floorAt > price) {
        ...
        if (strategy.parameters._buy && !strategy.parameters._btd && !strategy.parameters._buyTwap) {
            ...
            emit BuyExecuted(strategyId, price, slippage, toTokenAmount, rate, block.timestamp);
        } else if (strategy.parameters._btd) {
            console.log("BTD event emitted");
            emit BTDExecuted(strategyId, price, slippage, toTokenAmount, rate, block.timestamp);
    ...
    function setSellFloorValues(uint256 strategyId) internal {
        ...
        if (strategy.parameters._floor && strategy.parameters._floorType == FloorLegType.DECREASE_BY) {
            uint256 floorPercentage = LibTrade.MAX_PERCENTAGE - strategy.parameters._floorValue;
            strategy.floorAt = (strategy.investPrice * floorPercentage) / LibTrade.MAX_PERCENTAGE;
        }

        if (strategy.parameters._sell && strategy.parameters._sellType == SellLegType.INCREASE_BY) {
            uint256 sellPercentage = LibTrade.MAX_PERCENTAGE + strategy.parameters._sellValue;
            strategy.sellAt = (strategy.investPrice * sellPercentage) / LibTrade.MAX_PERCENTAGE;
        }
    }
    ...
    function checkRoundDataMistmatch(
        ...
        if (
            (strategy.parameters._btdType == DIP_SPIKE.FIXED_INCREASE && btdValue >= toFromPriceDifference) ||
            (strategy.parameters._btdType == DIP_SPIKE.FIXED_DECREASE && btdValue >= fromToPriceDifference) ||
