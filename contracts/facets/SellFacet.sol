...
contract SellFacet is Modifiers {
    ...
    function checkRoundDataMistmatch(
        ...
        if (strategy.parameters._strType == DIP_SPIKE.FIXED_INCREASE) {
            if (!(int256(strategy.parameters._strValue) >= int256(toPrice - fromPrice))) {
                revert();
            }
        } else if (strategy.parameters._strType == DIP_SPIKE.FIXED_DECREASE) {
            if (!(int256(strategy.parameters._strValue) >= int256(fromPrice - toPrice))) {
                revert();
            }
        } else if (strategy.parameters._strType == DIP_SPIKE.INCREASE_BY) {
            if (!(int256(strategy.parameters._strValue) >= (int256(toPrice - fromPrice) / int256(fromPrice)))) {
                revert();
            }
        } else if (strategy.parameters._strType == DIP_SPIKE.DECREASE_BY) {
            if (!(int256(strategy.parameters._strValue) >= (int256(fromPrice - toPrice) / int256(fromPrice)))) {
                revert();
            }
        }
    }
