  ...
  * @param swap The Swap struct containing address of the decentralized exchange (DEX) and calldata containing data for interacting with the DEX during the execution.
   ...
   */
    function executeSellTwap(uint256 strategyId, Swap calldata swap) external {
        ...
        if (!LibTime.getTimeDifference(block.timestamp, strategy.sellTwapExecutedAt, timeToExecute)) {
        ...
        transferSell(strategyId, value, swap, price, investRoundId, stableRoundId, strategy.sellAt);
   ...
   */
    function executeSTR(
        ...
        transferSell(strategyId, value, swap, price, investRoundId, stableRoundId, price);
    ...
    function executionSellValue(uint256 strategyId) public view returns (uint256) {
        uint256 value;
        Strategy storage strategy = s.strategies[strategyId];

        if (strategy.parameters._sellDCAUnit == DCA_UNIT.FIXED) {
            value = (strategy.parameters._investAmount > strategy.parameters._sellDCAValue)
                ? strategy.parameters._sellDCAValue
                : strategy.parameters._investAmount;
        } else if (strategy.parameters._sellDCAUnit == DCA_UNIT.PERCENTAGE) {
            value = (strategy.parameters._investAmount > strategy.sellPercentageAmount)
    ...
    function transferSell(
        ...
        uint256 sum = strategy.parameters._stableAmount + strategy.parameters._investAmount * strategy.investPrice;
        ...
        if (
            ...
            emit SellExecuted(strategyId, sellValue, slippage, toTokenAmount, rate, block.timestamp);
        } else if (strategy.parameters._str) {
            emit STRExecuted(strategyId, price, slippage, toTokenAmount, rate, block.timestamp);
    ...
    function checkRoundDataMistmatch(
        ...
        if (
            (strategy.parameters._strType == DIP_SPIKE.FIXED_INCREASE && strValue >= int256(toPrice - fromPrice)) ||
            (strategy.parameters._strType == DIP_SPIKE.FIXED_DECREASE && strValue >= int256(fromPrice - toPrice)) ||
            (strategy.parameters._strType == DIP_SPIKE.INCREASE_BY &&
                strValue >= (int256(toPrice - fromPrice) * 10000) / int256(fromPrice)) ||
